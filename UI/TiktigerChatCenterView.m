...
static NSString * const TiktigerChatFeatureID = @"chat.center";

@interface TiktigerChatCenterView ()
...
@implementation TiktigerChatCenterView
...
- (void)buildView {
    ...
    _scrollView.accessibilityIdentifier = @"tiktiger.chat.scroll";
    ...
    title.accessibilityIdentifier = @"tiktiger.chat.title";

    UILabel *subtitle = [[UILabel alloc] initWithFrame:CGRectZero];
    subtitle.translatesAutoresizingMaskIntoConstraints = NO;
    subtitle.text = @"A calm, transparent place for chat configuration and module health.";
    subtitle.font = [TiktigerDesignTokens bodyFont];
    subtitle.textColor = [TiktigerDesignTokens vipWhiteSecondary];
    subtitle.numberOfLines = 0;
    subtitle.adjustsFontForContentSizeCategory = YES;
    subtitle.accessibilityIdentifier = @"tiktiger.chat.subtitle";
    ...
    _statusCard = [[TiktigerGlassCard alloc] initWithTitle:@"Chat Status"];
    _statusCard.accessibilityIdentifier = @"tiktiger.chat.status-card";
    [_statusCard setElevated:YES];
    [_statusCard setStatusMessage:@"Waiting for Chat Module state."];
    ...
    _statusLabel.accessibilityIdentifier = @"tiktiger.chat.status";
    ...
    _statusDetailLabel.accessibilityIdentifier = @"tiktiger.chat.status-detail";
    [_statusCard.contentView addSubview:_statusLabel];
    [_statusCard.contentView addSubview:_statusDetailLabel];

    _featureCard = [[TiktigerGlassCard alloc] initWithTitle:@"Chat Feature Cards"];
    _featureCard.accessibilityIdentifier = @"tiktiger.chat.feature-card";
    [_featureCard setStatusMessage:@"A live overview of the four chat configuration domains."];
    ...
    _settingsCard = [[TiktigerGlassCard alloc] initWithTitle:@"Chat Configuration"];
    _settingsCard.accessibilityIdentifier = @"tiktiger.chat.settings-card";
    [_settingsCard setStatusMessage:@"Changes are validated and stored as configuration only; no chat transport behavior is added."];
    ...
    _diagnosticsCard = [[TiktigerGlassCard alloc] initWithTitle:@"Chat Diagnostics"];
    _diagnosticsCard.accessibilityIdentifier = @"tiktiger.chat.diagnostics-card";
...
- (TiktigerGlassRow *)chatRowWithTitle:(NSString *)title detail:(NSString *)detail icon:(NSString *)icon identifier:(NSString *)identifier {
...
- (void)setFeatureBinding:(id<TiktigerFeatureBinding>)binding {
    ...
    self.eventToken = [binding subscribeToModuleEvents:^(NSDictionary<NSString *,id> *event) {
        ...
        if (self != nil && [event[@"featureID"] isEqual:TiktigerChatFeatureID]) {
            [self applyChatPresentation:event[@"chat"] ?: [binding chatPresentationState]];
        }
    }];
    [self applyChatPresentation:binding != nil ? [binding chatPresentationState] : @{}];
}
...
- (void)applyChatPresentation:(NSDictionary<NSString *,id> *)snapshot {
    if (![NSThread isMainThread]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{ [weakSelf applyChatPresentation:copy]; });
        return;
    }
    ...
    [self.statusCard setStatusMessage:healthy ? @"Chat configuration is valid; behavior remains configuration-only." : @"Chat configuration is using a safe review state."];
    [self refreshFeatureCardsWithSnapshot:self.lastSnapshot];
    [self refreshSettingsWithSnapshot:self.lastSnapshot];
    [self refreshDiagnosticsWithSnapshot:self.lastSnapshot];
...
- (NSArray<NSDictionary<NSString *, NSString *> *> *)chatDefinitions {
    return @[
        @{ @"key": @"messagePrivacy", @"title": @"Message Privacy", @"detail": @"Stores message privacy preferences for future host-controlled enforcement.", @"icon": @"lock.message" },
        @{ @"key": @"chatControls", @"title": @"Chat Controls", @"detail": @"Stores validated chat control preferences without changing transport behavior.", @"icon": @"slider.horizontal.3" },
        @{ @"key": @"conversationSettings", @"title": @"Conversation Settings", @"detail": @"Stores conversation preferences as a safe configuration foundation.", @"icon": @"bubble.left.and.bubble.right" },
        @{ @"key": @"userPreferences", @"title": @"User Preferences", @"detail": @"Stores chat-related user preferences with schema validation.", @"icon": @"person.crop.circle" }
    ];
}
...
- (void)refreshFeatureCardsWithSnapshot:(NSDictionary<NSString *,id> *)snapshot {
    ...
    for (NSDictionary *definition in [self chatDefinitions]) {
        ...
        [self.featureStack addArrangedSubview:[self chatRowWithTitle:definition[@"title"] detail:detail icon:definition[@"icon"] identifier:[NSString stringWithFormat:@"tiktiger.chat.feature.%@", definition[@"key"]]]];
    }
}
...
- (void)refreshSettingsWithSnapshot:(NSDictionary<NSString *,id> *)snapshot {
    ...
    for (NSDictionary *definition in [self chatDefinitions]) {
        ...
        row.accessibilityIdentifier = [NSString stringWithFormat:@"tiktiger.chat.setting.%@", key];
        row.accessibilityHint = @"Toggle the validated chat configuration setting.";
        ...
        toggle.accessibilityIdentifier = [NSString stringWithFormat:@"tiktiger.chat.toggle.%@", key];
        toggle.accessibilityValue = toggle.isOn ? @"On" : @"Off";
        [toggle addTarget:self action:@selector(chatToggleChanged:) forControlEvents:UIControlEventValueChanged];
...
- (void)chatToggleChanged:(TiktigerGlassToggle *)toggle {
    if (self.featureBinding == nil) { [toggle setOn:!toggle.isOn animated:NO]; return; }
    NSString *prefix = @"tiktiger.chat.toggle.";
    ...
    NSError *error = nil;
    BOOL success = [self.featureBinding executeFeatureAction:@"updateChatSetting" featureID:TiktigerChatFeatureID payload:@{ @"key": key, @"value": @(toggle.isOn) } error:&error];
    if (!success) {
        [toggle setOn:!toggle.isOn animated:NO];
        toggle.accessibilityValue = @"Update failed";
        [self showToastMessage:error.localizedDescription ?: @"Chat setting was rejected safely."];
    } else {
        toggle.accessibilityValue = toggle.isOn ? @"On" : @"Off";
        [self showToastMessage:@"Chat configuration updated."];
    }
}
...
- (void)refreshDiagnosticsWithSnapshot:(NSDictionary<NSString *,id> *)snapshot {
    ...
    NSArray *rows = @[
        @[ @"Module status", moduleState, @"checkmark.shield", @"tiktiger.chat.diagnostics.module" ],
        @[ @"Configuration", configurationState, @"gearshape", @"tiktiger.chat.diagnostics.configuration" ],
        @[ @"Behavior", behaviorState, @"message", @"tiktiger.chat.diagnostics.behavior" ],
        @[ @"Last action", lastAction, @"clock.arrow.circlepath", @"tiktiger.chat.diagnostics.last-action" ],
        @[ @"Errors", [NSString stringWithFormat:@"%@", snapshot[@"errorCount"] ?: @0], @"exclamationmark.triangle", @"tiktiger.chat.diagnostics.errors" ]
    ];
    for (NSArray *definition in rows) { [self.diagnosticsStack addArrangedSubview:[self chatRowWithTitle:definition[0] detail:definition[1] icon:definition[2] identifier:definition[3]]]; }
}
